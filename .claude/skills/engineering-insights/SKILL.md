...
description: Reads and appends the touched module's INSIGHTS.md (client, server, reviewer-core, e2e). Use before answering a question about or changing code in one of those modules — read that file first; during a session the moment you hit something a future agent would otherwise relearn — a gotcha, a working approach, a dead-end antipattern, a codebase convention, a tool/library quirk, a recurring error+fix, or an open question; and again at session end, on wrap up / retro / what did we learn, or when /engineering-insights is invoked. Invoke the skill for the capture half rather than reading or appending the journal by hand — the entry format, the seven fixed sections and the append-only discipline live here, and "I want to record this / write this down so we don't hit it again / щоб більше не наступати" is a capture trigger, not a request to open a file. Never duplicates what INSIGHTS.md or CLAUDE.md already says, writes only substantial file-grounded entries, and is strictly append-only — never overwrites.
---
