...
| SPEC-05 | [`export-to-ci.md`](export-to-ci.md) | server, client, reviewer-core, agent-runner | implemented | Putting a tuned agent to work on a real repository: a four-step wizard generates the agent manifest, the skill files, a least-privilege GitHub Actions workflow and a self-contained runner, commits them as one atomic commit and opens a pull request the human reviews; once merged, the agent reviews every pull request in CI with the same engine and the same grounding gate, posts its findings back to GitHub, exits non-zero exactly when `ci_fail_on` trips, and the studio reads each run back — through the GitHub Actions API rather than any inbound endpoint — onto a CI Runs screen. Deliberately minimal — GitHub Actions only, read-only preview, no re-publish UI — with the security posture explicitly not reduced. |
| SPEC-06 | [`multi-agent-review.md`](multi-agent-review.md) | server, client | implemented | Fanning one pull request out to a chosen set of agents in one action, with the time and cost stated before the run from what past runs actually cost; a parent record grouping the runs; results as one column per agent or per-agent tabs with a finding detail; and every code location one agent flagged and another looked at and did not collapsed into one group carrying every agent's stance, including `did not flag`. Also makes the review executor's per-agent loop bounded-concurrent, which changes the existing all-agents fan-out too. |
