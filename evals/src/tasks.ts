 ...
 *
 * Safety: the read-only allow-list is NOT what makes this safe — `allowedTools` merely
 * auto-approves. `runClaude` passes `disallowedTools: MUTATING_TOOLS` on every session, which is
 * what actually keeps a bypassPermissions session from editing the repo it runs in.
 */
