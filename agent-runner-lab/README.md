...
> **ARCHIVED — this is not the runner the product ships.**
>
> This package is the CI runner vendored from `upstream/lesson-7-lab/agent-runner`
> in commit `7ed4240`, when branch `L07` began. The Export-to-CI worktree
> independently wrote a different runner for the same job — npm rather than pnpm,
> entered through `src/main.ts`, esbuild-bundled to a single committed
> `dist/runner.mjs` — and `server/src/platform/ci-runner.ts` copies **that** file
> into a target repository. Two packages cannot share one directory, so at the L07
> fan-out merge the feature's runner took `agent-runner/` and this one was moved
> here, renamed `@devdigest/agent-runner-lab`, and left otherwise verbatim.
>
> **Nothing imports it and no gate runs it.** It is kept because it is real work
> with its own journal, and because a reader who meets a `dist/index.js` or an
> `insights/INSIGHTS.md` reference elsewhere should be able to find what it names.
> It still builds: `pnpm typecheck` is clean and `pnpm test` is 23/23 against the
> post-merge `reviewer-core` and shared contracts, measured 2026-08-29.
>
> Everything below this line is as it was written, so it calls this package
> `agent-runner/` throughout — that was its path at the time. Read those as
> `agent-runner-lab/`. The live package is the sibling `agent-runner/`.
